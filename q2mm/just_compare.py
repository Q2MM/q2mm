...
def main(args):
    ...
    if "*" in args.calc[0]:
        args.calc = sorted(glob.glob(args.calc[0]))
    if "*" in args.ref[0]:
        args.ref = sorted(glob.glob(args.ref[0]))
    c_data = []
    r_data = []
    c_files:List[MacroModelLog] = map(MacroModelLog,args.calc)
    r_files = []# map(JaguarIn,args.ref)
    for r_file in args.ref:
        r_files.append((JaguarIn(r_file), JaguarOut(r_file[:-6]+'.out')))
    m_files:List[MacroModel] = [MacroModel(mmo_file) for mmo_file in args.mmo]
    m_structs:List[Structure] = []
    [m_structs.extend(m_file.structures) for m_file in m_files]
    for c_log, (r_in, r_out), m_struct in zip(c_files, r_files, m_structs):

        # dummies = m_struct.get_dummy_atom_indices()
        ...
        c_hess = c_log.hessian
        r_hess = r_in.get_hessian(m_struct.num_atoms)
        # if args.invert:
        #     r_hess = invert_ts_curvature(r_hess)
        r_evecs = r_out.eigenvectors
        mass_weight_eigenvectors(r_evecs, m_struct.atoms)
        mass_weight_hessian(r_hess, m_struct.atoms)


        #r_evals, r_evecs = np.linalg.eigh(r_hess)
        #c_evals, c_evecs = np.linalg.eigh(c_hess)

        # CALCULATED DATA TREATMENT FROM MJEIG in calculate.py
        c_eigenmatrix = np.dot(np.dot(r_evecs, c_hess), r_evecs.T)
        c_low_tri_idx = np.tril_indices_from(c_eigenmatrix)
        c_low_tri = c_eigenmatrix[c_low_tri_idx]
        c_data.extend([Datum(
                    val=e,
                    com='mjeig',
                    typ='eig',
                    src_1=c_log.filename,
                    src_2=r_in.filename,
                    idx_1=x + 1,
                    idx_2=y + 1)
                     for e, x, y in zip(
        ...
        if args.invert:
            r_eigenmatrix = replace_neg_eigenvalue(r_eigenmatrix)
        # Turn back into a full matrix.
        r_eigenmatrix = np.diag(r_eigenmatrix)
        low_tri_idx = np.tril_indices_from(r_eigenmatrix)
        low_tri = r_eigenmatrix[low_tri_idx]
        r_data.extend([ Datum(
                    val=e,
                    com='jeigz',
                    typ='eig',
                    src_1=r_out.filename,
                    src_2=r_in.filename,
                    idx_1=x + 1,
                    idx_2=y + 1)
                     for e, x, y in zip(
    ...
    score = compare_data(r_dict, c_dict, output=args.output, doprint=args.print)
...
def return_compare_parser():
    ...
    parser.add_argument(
        '--calc', '-c', type=str, metavar = '" Files with MM data"', nargs='+',
        help=('These file hold the FF data calculated from most recent TSFF.'))
    parser.add_argument(
        '--ref', '-r', type=str, metavar='" Files with QM data"', nargs='+',
        help=('These file hold the QM data calculated DFT.'))
    parser.add_argument(
        '--mmo', '-m', type=str, metavar='" Files with structural data"', nargs='+',
        help=('These file hold the structure data.'))
    parser.add_argument(
        '--invert', '-i', action='store_true', dest='invert',
        help='Invert reference hessian.')
    parser.add_argument(
