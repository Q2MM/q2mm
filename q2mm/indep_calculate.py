...
from just_compare import import_weights
import sch_util
    ...
    for filename, commands_for_filename in commands_for_filenames.items():
        ...
        if any(x in COM_MACROMODEL for x in commands_for_filename):
            if os.path.splitext(filename)[1] == '.mae':
                inps[filename] = schrod_indep_filetypes.Mae(
            ...
            if os.path.splitext(filename)[1] == '.chk':
                ...
                inps[com_filename] = schrod_indep_filetypes.GaussCom(
            ...
            if os.path.splitext(filename)[1] == '.xyz':
                inps[filename] = schrod_indep_filetypes.TinkerXYZ(
                    os.path.join(opts.directory, filename))
                inps[filename].commands = commands_for_filename
        # Gaussian to Tinker
            ...
            if os.path.splitext(filename)[1] == ".log":
                inps[filename] = schrod_indep_filetypes.TinkerXYZ_FOR_GAUS(
                    os.path.join(opts.directory, filename))
                inps[filename].commands = commands_for_filename
        # Gausssian to Amber
    ...
    with open(path, 'r') as f:
        for i, line in enumerate(f):
            ...
            assert len(cols) == 3, \
                ...
                i, path, line)
            lbl, wht, val = cols
            datum = schrod_indep_filetypes.Datum(lbl=lbl, wht=float(wht), val=float(val))
...
def collect_data(coms, inps, direc='.', sub_names=['OPT'], invert=None):
    ...
    for comma_filenames in filenames:
        ...
        ff = schrod_indep_filetypes.MM3(os.path.join(direc, name_fld))
        ff.import_ff()
    ...
    for idx_1, filenames in enumerate(filenames_s):
        temp = []
        for filename in filenames:
            mae = check_outs(filename, outs, schrod_indep_filetypes.Mae, direc)
            ...
            for idx_2, structure in enumerate(mae.structures):
                ...
                temp.append( schrod_indep_filetypes.Datum(
                        val=energy,
                        com='je',
                        typ='e',
                        src_1=filename,
    ...
    for idx_1, filenames in enumerate(filename_s):
        ...
        for filename in filenames:
            ...
            # how to write in new schrod_indep_filetypes. Notice how the new code utilizes
            # idx_2.
    ...
    for idx_1, filenames in enumerate(filenames_s):
        for filename in filenames:
            ...
            mae = check_outs(name_mae, outs, schrod_indep_filetypes.Mae, direc)
            ...
            selected_structures = schrod_indep_filetypes.select_structures(
                mae.structures, indices, ind)
            for idx_2, structure in selected_structures:
                data.append( schrod_indep_filetypes.Datum(
...
#            out = check_outs(
#                comma_sep_filenames, outs, schrod_indep_filetypes.AmberOut, direc)
...
#                idx_2=idx_2 + 1))
    ...
    for filename in filenames:
        ...
        name_hes = inps[filename].name_hes
        hes = check_outs(name_hes, outs, schrod_indep_filetypes.TinkerHess, direc)
        hes.natoms = num_atoms
        hess = hes.hessian
        schrod_indep_filetypes.mass_weight_hessian(hess, xyz_struct.atoms)
    ...
    for comma_filenames in filenames:
        ...
        xyz = check_outs(name_xyz, outs, schrod_indep_filetypes.Tinker_xyz, direc)
        xyz_hes = check_outs(name_xyz_hes, outs, schrod_indep_filetypes.TinkerHess, direc)
    ...
    for filename in filenames:
        ...
        structures = schrod_indep_filetypes.select_structures(
    ...
    for comma_filenames in filenames:
        ...
        gauss = check_outs(name_gauss_log, outs, schrod_indep_filetypes.GaussLog, direc)
    ...
    for filename in filenames:
        jin = check_outs(filename, outs, schrod_indep_filetypes.JaguarIn, direc)
        hess = jin.hessian
        schrod_indep_filetypes.mass_weight_hessian(hess, jin.structures[0].atoms)
        if invert:
    ...
    for filename in filenames:
        ...
        schrod_indep_filetypes.mass_weight_hessian(hess, log.structures[0].atoms)
        if invert:
    ...
    for filename in filenames:
        ...
        log = check_outs(name_log, outs, schrod_indep_filetypes.MacroModelLog, direc)
        hess = log.hessian
        dummies = mae.structures[0].get_dummy_atom_indices()
        hess_dummies = schrod_indep_filetypes.get_dummy_hessian_indices(dummies)
        hess = schrod_indep_filetypes.check_mm_dummy(hess, hess_dummies)
        low_tri_idx = np.tril_indices_from(hess)
        low_tri = hess[low_tri_idx]
    ...
    for comma_sep_filenames in filenames:
        ...
        jin = check_outs(name_in, outs, schrod_indep_filetypes.JaguarIn, direc)
        out = check_outs(name_out, outs, schrod_indep_filetypes.JaguarOut, direc)
        hess = jin.hessian
        evec = out.eigenvectors
        schrod_indep_filetypes.mass_weight_hessian(hess, jin.structures[0].atoms)
        schrod_indep_filetypes.mass_weight_eigenvectors(evec, out.structures[0].atoms)
        try:
            eigenmatrix = np.dot(np.dot(evec, hess), evec.T)
        except ValueError:
            logger.warning('Matrices not aligned!')
        ...
        if invert:
            schrod_indep_filetypes.replace_minimum(eigenmatrix, value=invert)
    ...
    for comma_filenames in filenames:
        ...
        mae = check_outs(name_mae, outs, schrod_indep_filetypes.Mae, direc)
        mae_log = check_outs(name_mae_log, outs, schrod_indep_filetypes.MacroModelLog, direc)
        gau_log = check_outs(name_gau_log, outs, schrod_indep_filetypes.GaussLog, direc)
    ...
    return np.array(data, dtype= schrod_indep_filetypes.Datum)
...
def collect_structural_data_from_mae(
    ...
    to the classes inside schrod_indep_filetypes.
    """
    data = []
    ...
    mmo = check_outs(name_mmo, outs, schrod_indep_filetypes.MacroModel, direc)
    selected_structures = schrod_indep_filetypes.select_structures(
...
def collect_structural_data_from_tinker_log(
    ...
    log = check_outs(name_log, outs, schrod_indep_filetypes.TinkerLog, direc)
    ...
    if com in ['te','teo','tea','teao']:
        ...
        new_datum = ( schrod_indep_filetypes.Datum(
            val=energy,
            typ=typ,
            src_1=name_log,
            idx_1=idx_1 + 1))
...
def collect_structural_data_from_amber_geo(
    ...
    log = check_outs(name_geo, outs, schrod_indep_filetypes.AmberGeo, direc) # returns classtype
    log_structure = log.structures
    struct = None
    if len(inps) == 1:
...
def collect_structural_data_from_amber_ene(
    ...
    log = check_outs(name_ene, outs, schrod_indep_filetypes.AmberEne, direc) # returns classtype
    log_structure = log.structures
    struct = None
    if len(inps) == 1:
        struct = log_structure[0]
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if len(cols) == 3:
                ...
                lbl_to_data_attrs(datum, lbl)
                data.append(datum)
    data = data.sort(key= schrod_indep_filetypes.datum_sort_key)
    return np.array(data)
