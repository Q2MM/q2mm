...
def plot_scores(starting_scores:pd.DataFrame, starting_score:float, scored_runs:list, final_scores:list, title:str):
    fig, ax = plt.subplots(1, len(scored_runs)+1, figsize=(25, 8))
    fig.suptitle(title)
...
def get_ff_params(base_direc:str, directories:list, final_scores:list, bond_rows:list, angle_rows:list, title:str=''):# -> tuple[list, list]:
    # Plot FCs
    ...
    for directory, score in zip(directories, final_scores):
        bonds.append(pd.read_csv(os.path.join(base_direc, directory, 'rh_qf_pso_end.fld'), skiprows=lambda x: x not in bond_rows, delim_whitespace=True, names=bond_cols).assign(FF=score))
        angles.append(pd.read_csv(os.path.join(base_direc, directory, 'rh_qf_pso_end.fld'), skiprows=lambda x: x not in angle_rows, delim_whitespace=True, names=angle_cols).assign(FF=score))
